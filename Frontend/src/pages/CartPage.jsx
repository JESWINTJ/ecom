...
function CartPage() {
  ...
  // Initialize cartItem from navigation state if available, otherwise null
  // The 'product' object is expected to be passed from ProductDetailPage
  const [cartItem, setCartItem] = useState(null); // Will be set by useEffect
  const [quantity, setQuantity] = useState(1); // Manage quantity separately

  // Price details calculation (still simplified/fixed for now, but uses dynamic item price)
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.product) {
      ...
      setQuantity(1); // Default quantity to 1 when a new product is added
    }
  }, [location.state]); // Re-run when navigation state changes

  const calculateTotal = () => {
    if (!cartItem) return 0;
    const itemPrice = parseFloat(cartItem.amount) * quantity; // Use product.amount and dynamic quantity
  ...
  const handleBuyNow = () => {
    ...
    console.log('Buying items in cart:', { ...cartItem, quantity });
    // In a real app, you'd send this to a cart service/backend
    // Pass cartItem and price details via navigate state to CheckoutPage
    navigate('/checkout', {
      state: {
        cartData: { ...cartItem, quantity }, // Pass the dynamic item with its current quantity
        // discountAmount: discount,
        couponsAmount: coupons,
        platformFeeAmount: platformFee,
        totalAmount: calculateTotal(),
        savedAmount: (parseFloat(cartItem.amount) * quantity) - calculateTotal() // Use dynamic price
      }
    });
  };
        ...
        {cartItem ? (
          <>
            ...
            <div className="flex flex-col items-center md:w-1/3 p-4 bg-white rounded-xl shadow-lg">
              <img
                src={cartItem.product_image} // Use product_image from the dynamic product object
                alt={cartItem.name}
            ...
            <div className="flex-grow p-4 bg-white rounded-xl shadow-lg">
              ...
              <div className="space-y-2 text-gray-700">
                <div className="flex justify-between">
                  ...
                  <span>₹{parseFloat(cartItem.amount) * quantity}</span> {/* Use dynamic price */}
                </div>
                <div className="flex justify-between">
                  <span>Discount</span>
